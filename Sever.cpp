...
void* handling_client(void*);

bool check_guess(const std::vector<int>& guess);
std::string create_result_message(const std::vector<int>& guess);
...
void* handling_client(void* args){
    ...
    while ((str_length = read(client_sock, message, BUFSIZ - 1)) != 0){
        ...
        if (check_guess(guess)){
            ...
            std::string result_message = create_result_message(guess);
            sending_message(result_message.c_str(), result_message.length());
        }
    }
    --client_count;
    pthread_mutex_unlock(&mutex_key);
...
std::string create_result_message(const std::vector<int>& guess)
{
    ...
    std::string result_message = "Strikes: " + std::to_string(strikes) + ", Balls: " + std::to_string(balls) + "\n\n";
